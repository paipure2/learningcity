...
class JetEngineCPTImport {
	...
	public static function getInstance() {		
		if (JetEngineCPTImport::$instance == null) {
			...
			JetEngineCPTImport::$media_instance = MediaHandling::getInstance();
		}
	...
	public function jet_engine_cpt_import_function($data_array, $type, $pID ,$mode, $hash_key,$line_number,$header_array,$value_array,$gmode,$templatekey) 
	{
		...
		foreach ($data_array as $dkey => $dvalue) {
			if(array_key_exists($dkey,$jet_data['JECPT'])){
				if (array_key_exists($dkey, $jet_data['JECPT']) && $jet_data['JECPT'][$dkey]['type'] === 'advanced-date') {
					$meta_value = [];
				...
				else if($jet_data['JECPT'][$dkey]['type'] == 'gallery' || $jet_data['JECPT'][$dkey]['type'] == 'media'){
					...
					foreach ( $exploded_gallery_items as $gallery ) {
						...
						if ( preg_match_all( '/\b(?:(?:https?|ftp|file):\/\/|www\.|ftp\.)[-A-Z0-9+&@#\/%=~_|$?!:,.]*[A-Z0-9+&@#\/%=~_|$]/i', $gallery ) ) {
							...
							$imgformat = $jet_data['JECPT'][$dkey]['value_format']; 
							$media_instance->store_image_ids($i=1);
							$get_gallery_id = $media_instance->image_meta_table_entry($line_number,$data_array, $pID, $field_name,$gallery, $hash_key, $plugin,$get_import_type, $templatekey, $gmode, $header_array, $value_array,$imgformat,'',$indexs);				
							if ( $get_gallery_id != '' ) {
				...
				elseif($jet_data['JECPT'][$dkey]['type'] == 'checkbox'){
					...
					if($jet_data['JECPT'][$dkey]['is_array'] == 1){
						...
						foreach($dvalexp as $dvalkey => $dvalueval){
							...
							// 	}	
							// }
							if(!empty($dvalueval) && in_array($dvalueval,$keys)){
								$arr[$dvalueval] = 'true';
							}		
						...
						if($jet_rf_data['JECPTRF'][$dkey]['type'] == 'gallery' || $jet_rf_data['JECPTRF'][$dkey]['type'] == 'media'){
							...
							foreach ( $exploded_gallery_items as $gallery ) {
								...
								if ( preg_match_all( '/\b(?:(?:https?|ftp|file):\/\/|www\.|ftp\.)[-A-Z0-9+&@#\/%=~_|$?!:,.]*[A-Z0-9+&@#\/%=~_|$]/i', $gallery ) ) {
									...
									$imgformat = $jet_rf_data['JECPTRF'][$dkey]['value_format'];
									$media_instance->store_image_ids($i=1);
									$get_gallery_id = $media_instance->image_meta_table_entry($line_number,$data_array, $pID, $field_name,$gallery, $hash_key, $plugin,$get_import_type, $templatekey, $gmode, $header_array, $value_array,$imgformat,'',$indexs);																									
									if ( $get_gallery_id != '' ) {
	...
	public function JetEngineCPTRFFields($type){
		...
		}//added new checkbox values for repeater
		if(isset($keys) && !in_array($dvalueval, $keys)){
			$get_meta_fields = $wpdb->get_results("SELECT id, meta_fields FROM {$wpdb->prefix}jet_post_types WHERE slug = '$type' AND status IN ('publish','built-in')");
			if(isset($get_meta_fields[0])){
				$unserialized_meta = maybe_unserialize($get_meta_fields[0]->meta_fields);
				$jet_engine_id = $get_meta_fields[0]->id;
				...
				if(!empty($unserialized_meta)){
					foreach($unserialized_meta as $jet_keys => $jet_values){
						if($jet_values['type'] == 'repeater' && $jet_values['name'] == $dkey){
							
							foreach($jet_values['repeater-fields'] as $jet_value_rep){
								if($jet_value_rep['type'] == 'checkbox'){
									$count_jetrepvalues = 0;
									$count_jetrepvalues = count($jet_value_rep['options']);
								
									$unserialized_meta[$jet_keys]['options'][$count_jetrepvalues]['key'] = $dvalueval;
									$unserialized_meta[$jet_keys]['options'][$count_jetrepvalues]['value'] = $dvalueval;
									$unserialized_meta[$jet_keys]['options'][$count_jetrepvalues]['id'] = $jet_value_rep['options'][$count_jetrepvalues - 1]['id'] + 1;
								}
							}
						}
					}
