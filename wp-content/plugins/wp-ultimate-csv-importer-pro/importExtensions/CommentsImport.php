...
if (!empty($data_array['comment_post_ID'])) {
    $post_id = (int) $data_array['comment_post_ID'];
} elseif (!empty($data_array['post_title'])) {
    $post = get_page_by_title($data_array['post_title'], OBJECT, 'product'); // Assuming you're dealing with WooCommerce products
    if ($post) {
        $post_id = $post->ID;
        $data_array['comment_post_ID'] = $post_id;
    } else {
        $core_instance->detailed_log[$line_number]['Message'] = "Skipped, post not found for title.";
    ...
    $core_instance->detailed_log[$line_number]['Message'] = "Skipped, no post ID or title.";
    $core_instance->detailed_log[$line_number]['state'] = 'Skipped';
    $wpdb->get_results("UPDATE $log_table_name SET skipped = $skipped_count WHERE $unikey_name = '$unikey_value'");
    return ['MODE' => 'Skipped'];
}
...
// Fallback if post_id couldn't be resolved
if (empty($post_id)) {
    $core_instance->detailed_log[$line_number]['Message'] = "Skipped, post ID could not be resolved.";
    $core_instance->detailed_log[$line_number]['state'] = 'Skipped';
    $wpdb->get_results("UPDATE $log_table_name SET skipped = $skipped_count WHERE $unikey_name = '$unikey_value'");
    return ['MODE' => 'Skipped'];
}
