...
function slimseo_import_function($data_array, $importas, $pID, $header_array, $value_array, $hash_key, $gmode, $templatekey, $line_number) {
    ...
    foreach ($mapping as $key => $aliases) {
        ...
        } elseif ($key === 'facebook_image' || $key === 'twitter_image') {
            $slimseo_fields[$key] = (string)$value;
        } elseif ($key === 'schema') {
            if (!empty($value)) {
                $decoded = json_decode($value, true);
                $slimseo_fields['schema'] = is_array($decoded) ? $decoded : maybe_unserialize($value);
            }
        } else {
            $slimseo_fields[$key] = (string)$value;
        }
    ...
    $slimseo_update = array_merge($existing, array_diff_key($slimseo_fields, ['schema' => '']));

    update_post_meta($pID, 'slim_seo', $slimseo_update);

    if (isset($slimseo_fields['schema'])) {
        update_post_meta($pID, 'slim_seo_schema', $slimseo_fields['schema']);
    }
