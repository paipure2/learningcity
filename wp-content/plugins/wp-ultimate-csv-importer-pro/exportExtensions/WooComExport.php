...
class WooCommerceExport
{
	...
	public function getWooComOrderData($id, $type = null, $optional = null)
	{
		...
		if ($order) {
			...
			// Fetch the raw meta
$esimgo_data_raw = get_post_meta($id, '_esimgo_data', true);

if (!empty($esimgo_data_raw)) {
    if (!is_array($esimgo_data_raw)) {
        $esimgo_data = maybe_unserialize($esimgo_data_raw);
    } else {
        $esimgo_data = $esimgo_data_raw;
    }

    if (!empty($esimgo_data['order'][0]['esims'][0]['iccid'])) {
        $iccid = $esimgo_data['order'][0]['esims'][0]['iccid'];
        WooCommerceExport::$export_instance->data[$id]['esim_iccid'] = $iccid;
    } 
} 
...
public function getQuizDataLifterLms($id) {
    global $wpdb;

    error_log("[LifterLMS Export] Starting export for quiz ID {$id}");
    ...
    $meta_keys = [
        ...
        '_llms_quiz',
        '_llms_questions_raw',
        '_llms_shuffle_answers',
        '_llms_negative_marking',
        '_llms_gradebook_enabled',
        '_llms_free_lesson',
  ...
  foreach ($meta_keys as $key) {
    $value = $wpdb->get_var("SELECT meta_value FROM {$wpdb->prefix}postmeta WHERE post_id = $id AND meta_key = '$key'");
    $unserialized_value = isset($value) ? maybe_unserialize($value) : '';

    if ($key === '_llms_lesson_id' && !empty($unserialized_value)) {
        $lesson_title = get_the_title((int) $unserialized_value);
        if (!empty($lesson_title)) {
            WooCommerceExport::$export_instance->data[$id][$key] = $lesson_title;
        } else {
            WooCommerceExport::$export_instance->data[$id][$key] = $unserialized_value;
        }
    } else {
        WooCommerceExport::$export_instance->data[$id][$key] = $unserialized_value;
    }
    ...
    $qid = $q->ID;
    $q_meta = get_post_meta($qid);

    $choices = [];
    foreach ($q_meta as $meta_key => $value) {
        if (strpos($meta_key, '_llms_choice_') === 0) {
            $choices[] = maybe_unserialize($value[0]);
        }
    }
    ...
    foreach ($choices as $choice) {
        ...
        $normalized = in_array(strtolower((string)$is_correct), ['1', 'yes', 'true'], true) ? '1' : '0';

        $choice_texts[] = $text;
        $correct_answers[] = $normalized;
    }

    $all_questions[] = implode('|', [
        $q->post_title,
        $q_meta['_llms_question_type'][0] ?? '',
        implode(',', $choice_texts),
        implode(',', $correct_answers),
    ]);

    $description_enabled[] = $q_meta['_llms_description_enabled'][0] ?? '';
    $multi_choices[]       = $q_meta['_llms_multi_choices'][0] ?? '';
    $points[]              = $q_meta['_llms_points'][0] ?? '';
    $video_enabled[]       = $q_meta['_llms_video_enabled'][0] ?? '';
    $video_src[]           = $q_meta['_llms_video_src'][0] ?? '';
}



    WooCommerceExport::$export_instance->data[$id]['_llms_questions'] = implode('||', $all_questions);
    WooCommerceExport::$export_instance->data[$id]['_llms_questions_raw'] = $wpdb->get_var(
        "SELECT meta_value FROM {$wpdb->prefix}postmeta WHERE post_id = $id AND meta_key='_llms_questions_raw'"
    );

    WooCommerceExport::$export_instance->data[$id]['_llms_description_enabled'] = implode('|', $description_enabled);
    WooCommerceExport::$export_instance->data[$id]['_llms_multi_choices']       = implode('|', $multi_choices);
    WooCommerceExport::$export_instance->data[$id]['_llms_points']              = implode('|', $points);
    WooCommerceExport::$export_instance->data[$id]['_llms_video_enabled']       = implode('|', $video_enabled);
    WooCommerceExport::$export_instance->data[$id]['_llms_video_src']           = implode('|', $video_src);
}
