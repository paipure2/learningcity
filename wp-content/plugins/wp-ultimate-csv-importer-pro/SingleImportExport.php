...
class SingleImportExport {
	public static $singlecsv_instance = null;
    ...
    if (strpos($key, 'meta_') === 0 && $value !== '') {
        ...
        } elseif ($meta_key === '_product_attributes') {
            $value = unserialize($value);
            update_post_meta($post_id, $meta_key, $value);

        } elseif (in_array($meta_key, ['_children', '_upsell_ids', '_crosssell_ids'], true)) {
            $value = unserialize($value);
            update_post_meta($post_id, $meta_key, $value);
...
foreach ($meta_keys as $meta_key) {
    if ($meta_key == '_product_attributes') {
        ...
        $row["meta_$meta_key"] = serialize($meta_value);
    }
    elseif ($meta_key == '_children' || $meta_key == '_upsell_ids' || $meta_key == '_crosssell_ids') {
        $meta_value = get_post_meta($post_id, $meta_key, true);
        $row["meta_$meta_key"] = serialize($meta_value);
    }
    else {
        ...
        if (is_array($meta_value) || is_object($meta_value)) {
            // Encode arrays/objects as JSON for safe export
            $meta_value = wp_json_encode($meta_value, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
        }

        // Use direct DB fetch to preserve original logic
        $row["meta_$meta_key"] = $wpdb->get_var(
            $wpdb->prepare(
        ...
        // If DB fetch failed (NULL), fall back to processed $meta_value
        if ($row["meta_$meta_key"] === null) {
