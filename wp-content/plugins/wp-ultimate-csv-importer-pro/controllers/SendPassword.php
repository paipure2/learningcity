...
class SendPassword {
	...
	public function doHooks(){
		...
		add_action('wp_ajax_get_sheet_setting', array($this,'showsettings'));
	}
	...
	$saved_key = get_option('openAI_settings');

	$ucisettings = get_option('sm_uci_pro_settings');

	$settings = array();
	if (is_array($ucisettings)) {
		foreach ($ucisettings as $key => $val) {
			if ($key == 'cmb2') {
				$settings[$key] = $val;
			} else {
	...
	$result['apikey']  = $saved_key ? $saved_key : '';
