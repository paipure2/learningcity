...
class Tables {
    ...
    update_option('sm_uci_pro_settings', $get_pro_settings);
		...
		$custom_fields_table = $wpdb->prefix ."smack_field_types";
		...
		$clitemplate = $wpdb->prefix . "clitemplate";
		...
		$result_1 = $wpdb->query("SHOW INDEXES FROM `{$wpdb->prefix}ultimate_csv_importer_scheduled_import` WHERE Key_name='isrun'");
		if($result_1 == 0){
			$wpdb->query("ALTER TABLE {$wpdb->prefix}ultimate_csv_importer_scheduled_import ADD INDEX `isrun`(`isrun`);");
		}

		$result_2 = $wpdb->query("SHOW INDEXES FROM `{$wpdb->prefix}ultimate_csv_importer_scheduled_export` WHERE Key_name='isrun'");
		if($result_2 == 0){
			$wpdb->query("ALTER TABLE {$wpdb->prefix}ultimate_csv_importer_scheduled_export ADD INDEX `isrun`(`isrun`);");
		}
		...
		$result = $wpdb->query("SHOW COLUMNS FROM `{$wpdb->prefix}ultimate_csv_importer_media_report` LIKE 'templatekey'");
		if($result == 0){
			$wpdb->query("ALTER TABLE `{$wpdb->prefix}ultimate_csv_importer_media_report` ADD COLUMN templatekey varchar(32)");
		}
		...
		$result_5 = $wpdb->query("SHOW COLUMNS FROM `{$wpdb->prefix}ultimate_csv_importer_media_report` LIKE 'id'");
		if($result_5 == 0){
			$wpdb->query("ALTER table `{$wpdb->prefix}ultimate_csv_importer_media_report` ADD COLUMN id int(10) NOT NULL AUTO_INCREMENT PRIMARY KEY;");
		}
		...
		$result_7 = $wpdb->query("SHOW COLUMNS FROM `{$wpdb->prefix}post_entries_table` LIKE 'ids'");
		if($result_7 == 0){
			$wpdb->query("ALTER table `{$wpdb->prefix}post_entries_table` ADD COLUMN ids int(10) NOT NULL AUTO_INCREMENT PRIMARY KEY;");
		}
		...
		$result_9 =  $wpdb->query("SHOW COLUMNS FROM `{$wpdb->prefix}ultimate_csv_importer_shortcode_manager` LIKE 'templatekey'");
		if($result_9 == 1){
		...
		$result_18 = $wpdb->query("SHOW COLUMNS FROM `{$wpdb->prefix}ultimate_csv_importer_scheduled_export` LIKE 'type'");
		if($result_18 == 0){
			$wpdb->query("ALTER TABLE `{$wpdb->prefix}ultimate_csv_importer_scheduled_export` ADD COLUMN type VARCHAR(255) DEFAULT NULL");
		}
		$result_19 = $wpdb->query("SHOW COLUMNS FROM `{$wpdb->prefix}ultimate_csv_importer_scheduled_export` LIKE 'query_data'");
		if($result_19 == 0){
			$wpdb->query("ALTER TABLE `{$wpdb->prefix}ultimate_csv_importer_scheduled_export` ADD COLUMN query_data VARCHAR(255) DEFAULT NULL ");
		}
		$result_20 = $wpdb->query("SHOW COLUMNS FROM `{$wpdb->prefix}ultimate_csv_importer_export_template` LIKE 'type'");
		if($result_20 == 0){
			$wpdb->query("ALTER TABLE `{$wpdb->prefix}ultimate_csv_importer_export_template` ADD COLUMN type VARCHAR(255) DEFAULT NULL");
		}
		$result_21 = $wpdb->query("SHOW COLUMNS FROM `{$wpdb->prefix}ultimate_csv_importer_export_template` LIKE 'query_data'");
		if($result_21 == 0){
			$wpdb->query("ALTER TABLE `{$wpdb->prefix}ultimate_csv_importer_export_template` ADD COLUMN query_data VARCHAR(255) DEFAULT NULL ");
		}
		$result_22 = $wpdb->query("SHOW COLUMNS FROM `{$wpdb->prefix}ultimate_csv_importer_export_template` LIKE 'client_mode'");
		if($result_22 == 0){
			$wpdb->query("ALTER table `{$wpdb->prefix}ultimate_csv_importer_export_template` ADD COLUMN client_mode varchar(50) DEFAULT NULL");
		}
		$result_23 = $wpdb->query("SHOW COLUMNS FROM `{$wpdb->prefix}ultimate_csv_importer_export_template` LIKE 'client_mode_values'");
		if($result_23 == 0){
			$wpdb->query("ALTER table `{$wpdb->prefix}ultimate_csv_importer_export_template` ADD COLUMN client_mode_values longtext DEFAULT NULL");
		}
	}
}