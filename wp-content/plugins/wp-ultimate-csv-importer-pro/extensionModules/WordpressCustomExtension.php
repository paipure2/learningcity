    ...
    public function processExtension($data) {		
        ...
        if($module != 'user') {   
            ...
            $keys = $wpdb->get_col( "SELECT pm.meta_key FROM {$wpdb->prefix}posts p
                ...
                if(is_plugin_active('jet-engine/jet-engine.php')){
                    ...
                    $keys = $wpdb->get_col( "SELECT pm.meta_key FROM {$wpdb->prefix}posts p
                                ...
                                AND meta_key NOT LIKE 'jet%' 
                                AND meta_key NOT LIKE 'wpcf-%' 
                                AND meta_key NOT LIKE 'wpcr3_%' 
                                AND meta_key NOT LIKE '%pods%' 
                                AND meta_key NOT LIKE '%group_%' 
                                AND meta_key NOT LIKE '%repeat_%' 
                                AND meta_key NOT LIKE 'mp_%' 
                                AND meta_key NOT LIKE 'rank_%' 
                                AND meta_key NOT LIKE '%jet-review%' 
                                $jetfields_not_like_query 
                                $jet_not_like_query
                                ORDER BY meta_key" );
            ...
            $keys = $wpdb->get_col( "SELECT um.meta_key FROM {$wpdb->prefix}users u
                                    ...
                                    and meta_key NOT LIKE 'ywbc_%' and meta_key NOT LIKE 'wp_%' and meta_key NOT LIKE 'wc_%' and meta_key NOT LIKE 'mp_%' and meta_key NOT LIKE 'shipping_%' and meta_key NOT LIKE 'show_%' and meta_key NOT LIKE 'acf_%' and meta_key NOT LIKE 'user_%' and meta_key NOT LIKE 'woocommerce_%' and meta_key NOT LIKE '{$wpdb->prefix}%'
                                    ORDER BY meta_key" );                             
        }
    ...
    public function extensionSupportedImportType($import_type){
        ...
        if(!is_plugin_active('wpml-import/plugin.php')){
            $active_plugin = array(
                ...
                "wp-e-commerce-custom-fields/custom-fields.php",
        ...
        if($import_type == 'Posts' || $import_type == 'Pages' || $import_type == 'CustomPosts' || $import_type == 'event' || $import_type == 'location' || $import_type == 'Users' || $import_type == 'WooCommerce' || $import_type == 'WPeCommerce') {
            return true;
        }
