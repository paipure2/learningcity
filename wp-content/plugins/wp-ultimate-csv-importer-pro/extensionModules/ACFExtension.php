...
class ACFExtension extends ExtensionHandler{
	...
	*/
	public function ACFFields($import_type ,$group) {	
		...
		$group_field_arr = [];
$repeater_field_arr = [];
$get_acf_group_fields = [];
$get_acf_repeater_fields = [];
			...
			if(!empty($rule)) {
				if ($import_type != 'Users') {
					...
					foreach($rule['location'] as $key => $value) {
						...
						else if( $value[0]['operator'] == '==' && $value[0]['param'] == 'user_form'){
							$group_id_arr[] = $group_rules->ID;
						}
					}
				}
		...
		if ( !empty($group_id_arr) ) {
			foreach($group_id_arr as $groupId) {
				...
				if(!empty($repeater_field_arr) && is_array($repeater_field_arr)){
	...
	*/
	public function extensionSupportedImportType($import_type){
		if(is_plugin_active('advanced-custom-fields/acf.php')|| is_plugin_active('secure-custom-fields/secure-custom-fields.php')|| is_plugin_active('acf-repeater/acf-repeater.php')){
