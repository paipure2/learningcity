...
class ElementorExtension extends ExtensionHandler{
	...
	public function processExtension($data){  
		...
		if( is_plugin_active('elementor/elementor.php') || is_plugin_active('elementor-pro/elementor-pro.php') ){   
			if($import_type == 'elementor_library' || $import_type == 'Posts' || $import_type == 'Pages' || $import_type == 'WooCommerce' || $import_type == 'CustomPosts'){
	...
	public function extensionSupportedImportType($import_type ){
		$import_type = $this->import_name_as($import_type);
		if( is_plugin_active('elementor/elementor.php') || is_plugin_active('elementor-pro/elementor-pro.php')){
			if($import_type == 'elementor_library' || $import_type == 'Pages' || $import_type == 'Posts' || $import_type == 'WooCommerce' || $import_type == 'CustomPosts') { 
				return true;
			}else{
				return false;
			}
