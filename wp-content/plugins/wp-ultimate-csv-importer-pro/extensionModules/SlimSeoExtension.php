...
class SlimSeoExtension extends ExtensionHandler {
    ...
    $slimseoProFields = array(	
        'Schema'    => 'schema'
    );

    $free_active = is_plugin_active('slim-seo/slim-seo.php');
    $pro_active  = (
        is_plugin_active('slim-seo-pro/slim-seo.php') ||
    ...
    if ($free_active && !$pro_active) {
        $final = $slimseoFields;
    } elseif (!$free_active && $pro_active) {
        $final = $slimseoProFields;
    } else {
        $final = array_merge($slimseoFields, $slimseoProFields);
    }

    $mapping_array = $this->convert_static_fields_to_array($final);

    return [
        'slim_seo_fields' => $mapping_array
    ];
}
