...
class FtpUpload implements Uploads{
    ...
    private function check_ftp_security() {
        ...
        if (!wp_verify_nonce($_POST['securekey'], 'smack-ultimate-csv-importer-pro')) {
    ...
    public function upload_function(){
        ...
        $fields = [
            'HostPath' => 'FTP connection failed, Path is empty',
            'HostPort' => 'FTP connection failed, host_port is empty',
            'HostName' => 'FTP connection failed, host_name is empty',
            'HostUserName' => 'FTP connection failed, host_username is empty',
            'HostPassword' => 'FTP connection failed, host_password is empty'
        ];
        
        // Check for empty fields and respond with appropriate error message
        ...
        $host_name = str_replace(['ftp://', 'ftps://'], '', trim($host_name));
        ...
        if(!$conn_id){
            ...
            if($validate_format == 'yes'){
                ...
                if($upload_dir){
                    ...
                    clearstatcache();
                    $dld = $fileSize;
                    $dld_numeric = (float) preg_replace('/[^0-9.]/', '', $dld);
                    if ($fs != 0) {
                        $i = ($dld_numeric / $fs) * 100;
